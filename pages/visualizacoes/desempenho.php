<?php
require_once '../../includes/common/permissoes.php';
verificarPermissao(['secretaria', 'diretor_geral', 'diretor_pedagogico', 'coordenador']);
require_once '../../process/verificar_sessao.php';
require_once '../../database/conexao.php';

// Obter parâmetros do relatório
$ano_letivo = $_GET['ano_letivo'] ?? date('Y');
$trimestre = $_GET['trimestre'] ?? null;
$id_curso = $_GET['id_curso'] ?? null;
$id_turma = $_GET['id_turma'] ?? null;

// Construir consulta SQL para notas
$query_notas = "SELECT 
                n.*, 
                a.nome as aluno_nome, 
                t.nome as turma_nome, 
                c.nome as curso_nome,
                d.nome as disciplina_nome
              FROM nota n
              JOIN aluno al ON n.aluno_id_aluno = al.id_aluno
              JOIN usuario a ON al.usuario_id_usuario = a.id_usuario
              JOIN disciplina d ON n.disciplina_id_disciplina = d.id_disciplina
              JOIN turma t ON al.turma_id_turma = t.id_turma
              JOIN curso c ON t.curso_id_curso = c.id_curso
              WHERE al.ano_letivo = ?";

$params = [$ano_letivo];
$types = "i";

if ($trimestre) {
    $query_notas .= " AND n.trimestre = ?";
    $params[] = $trimestre;
    $types .= "i";
}

if ($id_curso) {
    $query_notas .= " AND t.curso_id_curso = ?";
    $params[] = $id_curso;
    $types .= "i";
}

if ($id_turma) {
    $query_notas .= " AND al.turma_id_turma = ?";
    $params[] = $id_turma;
    $types .= "i";
}

// Preparar e executar a consulta de notas
$stmt = $conn->prepare($query_notas);
if ($types) {
    $stmt->bind_param($types, ...$params);
}
$stmt->execute();
$result_notas = $stmt->get_result();
$notas = $result_notas->fetch_all(MYSQLI_ASSOC);

// Calcular médias por aluno, disciplina e turma
$medias_alunos = [];
$medias_disciplinas = [];
$medias_turmas = [];

foreach ($notas as $nota) {
    // Médias por aluno/disciplina
    $key_aluno = $nota['aluno_id_aluno'] . '_' . $nota['disciplina_id_disciplina'];
    if (!isset($medias_alunos[$key_aluno])) {
        $medias_alunos[$key_aluno] = [
            'aluno_nome' => $nota['aluno_nome'],
            'disciplina_nome' => $nota['disciplina_nome'],
            'turma_nome' => $nota['turma_nome'],
            'total' => 0,
            'peso_total' => 0,
            'quantidade' => 0
        ];
    }
    $medias_alunos[$key_aluno]['total'] += $nota['nota'] * $nota['peso'];
    $medias_alunos[$key_aluno]['peso_total'] += $nota['peso'];
    $medias_alunos[$key_aluno]['quantidade']++;
    
    // Médias por disciplina/turma
    $key_disciplina = $nota['disciplina_id_disciplina'] . '_' . $nota['turma_id_turma'];
    if (!isset($medias_disciplinas[$key_disciplina])) {
        $medias_disciplinas[$key_disciplina] = [
            'disciplina_nome' => $nota['disciplina_nome'],
            'turma_nome' => $nota['turma_nome'],
            'total' => 0,
            'peso_total' => 0,
            'quantidade' => 0,
            'alunos' => []
        ];
    }
    $medias_disciplinas[$key_disciplina]['total'] += $nota['nota'] * $nota['peso'];
    $medias_disciplinas[$key_disciplina]['peso_total'] += $nota['peso'];
    $medias_disciplinas[$key_disciplina]['quantidade']++;
    $medias_disciplinas[$key_disciplina]['alunos'][$nota['aluno_id_aluno']] = $nota['aluno_nome'];
    
    // Médias por turma
    if (!isset($medias_turmas[$nota['turma_id_turma']])) {
        $medias_turmas[$nota['turma_id_turma']] = [
            'turma_nome' => $nota['turma_nome'],
            'curso_nome' => $nota['curso_nome'],
            'total' => 0,
            'peso_total' => 0,
            'quantidade' => 0,
            'alunos' => []
        ];
    }
    $medias_turmas[$nota['turma_id_turma']]['total'] += $nota['nota'] * $nota['peso'];
    $medias_turmas[$nota['turma_id_turma']]['peso_total'] += $nota['peso'];
    $medias_turmas[$nota['turma_id_turma']]['quantidade']++;
    $medias_turmas[$nota['turma_id_turma']]['alunos'][$nota['aluno_id_aluno']] = $nota['aluno_nome'];
}

// Calcular médias finais
foreach ($medias_alunos as &$media) {
    $media['media'] = $media['peso_total'] > 0 ? $media['total'] / $media['peso_total'] : 0;
}

foreach ($medias_disciplinas as &$media) {
    $media['media'] = $media['peso_total'] > 0 ? $media['total'] / $media['peso_total'] : 0;
    $media['alunos_count'] = count($media['alunos']);
}

foreach ($medias_turmas as &$media) {
    $media['media'] = $media['peso_total'] > 0 ? $media['total'] / $media['peso_total'] : 0;
    $media['alunos_count'] = count($media['alunos']);
}

$title = "Relatório de Desempenho Acadêmico";
?>

<!DOCTYPE html>
<html lang="pt">
<head>
    <?php require_once '../../includes/common/head.php'; ?>
    <style>
        .report-header {
            margin-bottom: 30px;
            padding-bottom: 15px;
            border-bottom: 1px solid #eee;
        }
        .report-title {
            font-size: 24px;
            font-weight: bold;
        }
        .report-filters {
            font-size: 14px;
            color: #666;
        }
        .report-table {
            width: 100%;
            margin-bottom: 20px;
        }
        .report-table th {
            background-color: #f8f9fa;
            text-align: left;
            padding: 10px;
        }
        .report-table td {
            padding: 10px;
            border-bottom: 1px solid #eee;
        }
        .chart-container {
            margin: 30px 0;
            height: 400px;
        }
        .print-actions {
            margin-top: 20px;
            text-align: right;
        }
        @media print {
            .no-print {
                display: none;
            }
            body {
                padding: 20px;
                font-size: 12px;
            }
            .report-table {
                page-break-inside: avoid;
            }
            .chart-container {
                height: 300px;
            }
        }
    </style>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
</head>
<body>
    <div class="container-fluid">
        <div class="report-header">
            <div class="report-title">Relatório de Desempenho Acadêmico</div>
            <div class="report-filters">
                Ano Letivo: <?= $ano_letivo ?> | 
                <?= $trimestre ? "Trimestre: $trimestre | " : "" ?>
                <?= $id_curso ? "Curso: " . htmlspecialchars($medias_turmas[array_key_first($medias_turmas)]['curso_nome'] ?? '') . " | " : "" ?>
                <?= $id_turma ? "Turma: " . htmlspecialchars($medias_turmas[array_key_first($medias_turmas)]['turma_nome'] ?? '') : "" ?>
            </div>
        </div>
        
        <!-- Gráfico de Desempenho por Disciplina -->
        <div class="chart-container">
            <canvas id="chartDisciplinas"></canvas>
        </div>
        
        <!-- Tabela de Médias por Disciplina/Turma -->
        <h4>Médias por Disciplina</h4>
        <table class="report-table">
            <thead>
                <tr>
                    <th>Disciplina</th>
                    <th>Turma</th>
                    <th>Média Geral</th>
                    <th>Nº Alunos</th>
                    <th>Nº Avaliações</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($medias_disciplinas as $media): ?>
                <tr>
                    <td><?= htmlspecialchars($media['disciplina_nome']) ?></td>
                    <td><?= htmlspecialchars($media['turma_nome']) ?></td>
                    <td><?= number_format($media['media'], 2) ?></td>
                    <td><?= $media['alunos_count'] ?></td>
                    <td><?= $media['quantidade'] ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        
        <!-- Tabela de Médias por Turma -->
        <h4>Médias por Turma</h4>
        <table class="report-table">
            <thead>
                <tr>
                    <th>Turma</th>
                    <th>Curso</th>
                    <th>Média Geral</th>
                    <th>Nº Alunos</th>
                    <th>Nº Avaliações</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($medias_turmas as $media): ?>
                <tr>
                    <td><?= htmlspecialchars($media['turma_nome']) ?></td>
                    <td><?= htmlspecialchars($media['curso_nome']) ?></td>
                    <td><?= number_format($media['media'], 2) ?></td>
                    <td><?= $media['alunos_count'] ?></td>
                    <td><?= $media['quantidade'] ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        
        <!-- Tabela de Médias por Aluno -->
        <h4>Médias por Aluno</h4>
        <table class="report-table">
            <thead>
                <tr>
                    <th>Aluno</th>
                    <th>Turma</th>
                    <th>Disciplina</th>
                    <th>Média</th>
                    <th>Nº Avaliações</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($medias_alunos as $media): ?>
                <tr>
                    <td><?= htmlspecialchars($media['aluno_nome']) ?></td>
                    <td><?= htmlspecialchars($media['turma_nome']) ?></td>
                    <td><?= htmlspecialchars($media['disciplina_nome']) ?></td>
                    <td><?= number_format($media['media'], 2) ?></td>
                    <td><?= $media['quantidade'] ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        
        <div class="print-actions no-print">
            <button onclick="window.print()" class="btn btn-primary">
                <i class="feather icon-printer"></i> Imprimir/Salvar PDF
            </button>
            <a href="../compartilhados/relatorios.php" class="btn btn-secondary">
                <i class="feather icon-arrow-left"></i> Voltar
            </a>
            <!-- Opção para enviar para destinatários -->
            <button class="btn btn-success" data-toggle="modal" data-target="#enviarModal">
                <i class="feather icon-send"></i> Enviar para Destinatários
            </button>
        </div>
    </div>
    
    <!-- Modal para enviar relatório -->
    <div class="modal fade" id="enviarModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Enviar Relatório</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div id="alertDownload" class="alert alert-warning">
                        <i class="feather icon-alert-triangle"></i> Por favor, primeiro baixe o relatório usando o botão "Imprimir/Salvar PDF" antes de enviar.
                    </div>
                    
                    <form id="formEnviarRelatorio" method="POST" action="enviar_relatorio.php" enctype="multipart/form-data" style="display: none;">
                        <input type="hidden" name="tipo" value="matriculas">
                        <input type="hidden" name="ano_letivo" value="<?= $ano_letivo ?>">
                        <input type="hidden" name="trimestre" value="<?= $trimestre ?>">
                        <input type="hidden" name="id_curso" value="<?= $id_curso ?>">
                        <input type="hidden" name="id_turma" value="<?= $id_turma ?>">
                        <input type="hidden" name="ordenacao" value="<?= $ordenacao ?>">
                        
                        <div class="form-group">
                            <label for="arquivo_pdf">Arquivo PDF do Relatório *</label>
                            <input type="file" class="form-control" id="arquivo_pdf" name="arquivo_pdf" accept=".pdf" required>
                        </div>
                        
                        <div class="form-group">
                            <label>Destinatários *</label>
                            <div class="checkbox-fade fade-in-primary">
                                <label>
                                    <input type="checkbox" name="destinatarios[]" value="tipo_usuario:diretor_geral">
                                    <span class="cr"><i class="cr-icon feather icon-check"></i></span>
                                    <span>Diretores Gerais</span>
                                </label>
                            </div>
                            <div class="checkbox-fade fade-in-primary">
                                <label>
                                    <input type="checkbox" name="destinatarios[]" value="tipo_usuario:diretor_pedagogico">
                                    <span class="cr"><i class="cr-icon feather icon-check"></i></span>
                                    <span>Diretores Pedagógicos</span>
                                </label>
                            </div>
                            <div class="checkbox-fade fade-in-primary">
                                <label>
                                    <input type="checkbox" name="destinatarios[]" value="tipo_usuario:coordenador">
                                    <span class="cr"><i class="cr-icon feather icon-check"></i></span>
                                    <span>Coordenadores</span>
                                </label>
                            </div>
                            <div class="checkbox-fade fade-in-primary">
                                <label>
                                    <input type="checkbox" name="destinatarios[]" value="tipo_usuario:professor">
                                    <span class="cr"><i class="cr-icon feather icon-check"></i></span>
                                    <span>Professores</span>
                                </label>
                            </div>
                            <div class="checkbox-fade fade-in-primary">
                                <label>
                                    <input type="checkbox" name="destinatarios[]" value="tipo_usuario:secretaria">
                                    <span class="cr"><i class="cr-icon feather icon-check"></i></span>
                                    <span>Secretaria</span>
                                </label>
                            </div>
                        </div>
                        
                        <div class="form-group">
                            <label for="mensagem">Mensagem (opcional)</label>
                            <textarea class="form-control" id="mensagem" name="mensagem" rows="3"></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                    <button type="button" id="btnEnviarRelatorio" class="btn btn-primary" disabled>
                        Enviar Relatório
                    </button>
                </div>
            </div>
        </div>
    </div>

    <?php require_once '../../includes/common/js_imports.php'; ?>
    
    <script>
        // Gráfico de desempenho por disciplina
        const ctxDisciplinas = document.getElementById('chartDisciplinas').getContext('2d');
        
        // Preparar dados para o gráfico
        const disciplinasLabels = [];
        const disciplinasData = [];
        
        <?php foreach ($medias_disciplinas as $media): ?>
            disciplinasLabels.push('<?= addslashes($media['disciplina_nome']) ?>');
            disciplinasData.push(<?= $media['media'] ?>);
        <?php endforeach; ?>
        
        const chartDisciplinas = new Chart(ctxDisciplinas, {
            type: 'bar',
            data: {
                labels: disciplinasLabels,
                datasets: [{
                    label: 'Média por Disciplina',
                    data: disciplinasData,
                    backgroundColor: 'rgba(54, 162, 235, 0.5)',
                    borderColor: 'rgba(54, 162, 235, 1)',
                    borderWidth: 1
                }]
            },
            options: {
                responsive: true,
                maintainAspectRatio: false,
                scales: {
                    y: {
                        beginAtZero: true,
                        max: 20,
                        title: {
                            display: true,
                            text: 'Média'
                        }
                    },
                    x: {
                        title: {
                            display: true,
                            text: 'Disciplinas'
                        }
                    }
                },
                plugins: {
                    title: {
                        display: true,
                        text: 'Desempenho Médio por Disciplina'
                    },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return `Média: ${context.raw.toFixed(2)}`;
                            }
                        }
                    }
                }
            }
        });
    </script>

        <script>
        // Verificar se o usuário já tentou baixar o relatório
        let tentouBaixar = false;
        
        // Quando clicar no botão de imprimir/salvar PDF
        document.querySelector('button[onclick="window.print()"]').addEventListener('click', function() {
            tentouBaixar = true;
            
            // Mostrar o formulário de envio e esconder o alerta
            document.getElementById('alertDownload').style.display = 'none';
            document.getElementById('formEnviarRelatorio').style.display = 'block';
            document.getElementById('btnEnviarRelatorio').disabled = false;
            document.getElementById('btnEnviarRelatorio').setAttribute('onclick', "document.getElementById('formEnviarRelatorio').submit()");
        });
        
        // Quando abrir o modal, verificar se já tentou baixar
        $('#enviarModal').on('show.bs.modal', function() {
            if (tentouBaixar) {
                document.getElementById('alertDownload').style.display = 'none';
                document.getElementById('formEnviarRelatorio').style.display = 'block';
                document.getElementById('btnEnviarRelatorio').disabled = false;
                document.getElementById('btnEnviarRelatorio').setAttribute('onclick', "document.getElementById('formEnviarRelatorio').submit()");
            } else {
                document.getElementById('alertDownload').style.display = 'block';
                document.getElementById('formEnviarRelatorio').style.display = 'none';
                document.getElementById('btnEnviarRelatorio').disabled = true;
                document.getElementById('btnEnviarRelatorio').removeAttribute('onclick');
            }
        });
    </script>
</body>
</html>