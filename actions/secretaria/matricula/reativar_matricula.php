...
try {
    ...
    $query = "UPDATE matricula SET status_matricula = 'ativa' WHERE id_matricula = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('i', $id_matricula);
    
    if ($stmt->execute()) {
        $response['success'] = true;
        $response['message'] = 'Matrícula reativada com sucesso';
    } else {
        throw new Exception('Erro ao reativar matrícula no banco de dados');
    }
} catch (Exception $e) {
