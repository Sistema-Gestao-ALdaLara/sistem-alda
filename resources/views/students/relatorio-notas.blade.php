<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Relatório de Notas do {{DB::table('trimestre')->where('id', $trimestre_id)->first()->nome_trimestre}} TRIMESTRE</title>
   
    
<style>
     table {
        font-family:  'Times New Roman', Times, serif;
        border-collapse: collapse;
        width: 100%;
    }
    
    td,th {
        border: 1px solid black;
        text-align: left;
        padding-left: 1px;
        
    }
    .line{
        text-align: center;
        
    }
    .img{
        display: inline-block;
        margin-left: 14%;
        margin-top: -15px;
    }

    footer{
        width: 100%;
        display: flex;
        flex-wrap: wrap;
        justify-content: flex-start;
        align-items: flex-start;
        align-content: flex-start;
    }

    #data {
            /* float: left;  */
            float: left;
            position: fixed;
            width: 380px;
        }

    #coordenador {
        text-align: center;
        margin-left: -270;
        float: left;
        position: fixed;
        bottom: -10;
        width: 250px;
    }
    #director-turma {
        text-align: center;
        float: right;
        position: fixed;
        bottom: -10;
        width: 250px;
    }
  
</style>
    
</head>

<body>
    <br>
        <div class="img">
            <br>
              <div class="line">
                <img src='data:image/png;base64,'/>
                <br>
                <br>
                    INSTITUTO MÉDIO INSDUTRIAL SIMIONE MUCUNE Nº 1119 <br>
                    SUBDIRECÇÃO PEDAGÓGICA <br>
                    CURSO: <b>{{mb_strtoupper(App\Models\Curso::pegarCurso($dados_turma->curso_id)->nome)}}</b><br>
                    <b>RELATÓRIO DE NOTAS DO {{DB::table('trimestre')->where('id', $trimestre_id)->first()->nome_trimestre}} TRIMESTRE / ANO LECTIVO {{session('ano_em_curso')}}</b>
                <br>
                <br>
                <br>
                
              </div>
        </div>
        <table>
            <thead>
                <tr>
                    <th rowspan="5"></th>
                    <th colspan="5" style="text-align: center">Dados do Aluno</th>
                </tr>
                <tr>
                    <td colspan="3">Nome: <b style="color: red">{{$aluno->nome_cand}}</b></td>
                    <td style="border-right: 1.2px transparent;">Classe: <b>{{App\Models\Classe::pegarClasse($dados_turma->classe_id)->nome_classe}}</b></td>
                    <td></td>
                </tr>
                <tr>
                    <td>Nº de Processo: <b>{{$aluno->numero_processo}}</b></td>
                    <td colspan="2">Nº Convencional: <b>{{$aluno->numero_aluno}}</b></td>
                    <td style="width: 115px;border-right: 1.2px transparent;">Turma: <b>{{$dados_turma->turma}}</b></td>
                    <td></td>
                </tr>
                <tr>
                    <th rowspan="2" style="text-align: center">Disciplinas</th>
                    <th colspan="4" style="text-align: center">Notas</th>
                </tr>
                    <td>MAC</td>
                    <td>NPP</td>
                    <td>NPT</td>
                    <td style="text-align: center">MT</td>
                <tr>

                </tr>
            </thead>
            <tbody>
                @foreach($disciplinas as $disciplina)
                   <tr>
                        <td style="text-align: center">{{++$contagem}}</td>
                        <td>{{$disciplina->nome_disciplina}}</td>

                        {{--MAC--}}
                        @if(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id) != null)
                            @if (App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mac >= 10)
                                <td style="text-align: center; color: blue">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mac}}</td>
                            @elseif(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mac < 10)
                                <td style="text-align: center; color: red">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mac}}</td>
                            @endif
                        @else
                            <td style="text-align: center;">-</td>
                        @endif

                        {{--NPP--}}
                        @if(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id) != null)
                            @if (App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npp >= 10)
                                <td style="text-align: center; color: blue">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npp}}</td>
                            @elseif(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npp < 10)
                                <td style="text-align: center; color: red">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npp}}</td>
                            @endif
                        @else
                            <td style="text-align: center;">-</td>
                        @endif

                        {{--NPT--}}
                        @if(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id) != null)
                            @if (App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npt >= 10)
                                <td style="text-align: center; color: blue">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npt}}</td>
                            @elseif(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npt < 10)
                                <td style="text-align: center; color: red">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->npt}}</td>
                            @endif
                        @else
                            <td style="text-align: center;">-</td>
                        @endif

                        {{--MT--}}
                        @if(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id) != null)
                            @if (App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mt >= 10)
                                <td style="text-align: center; color: blue">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mt}}</td>
                            @elseif(App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mt < 10)
                                <td style="text-align: center; color: red">{{App\Models\ProvasTrimestre::buscarNotas($trimestre_id, session('aluno_id'), $disciplina->disciplina_id)->mt}}</td>
                            @endif
                        @else
                            <td style="text-align: center;">-</td>
                        @endif
                        
                    
                   </tr>
                @endforeach
            </tbody>
        </table>
    </div>

    <footer>
        <div id="data">
            <br>
            &nbsp; LUANDA AOS ______/__________________/_______
        </div>

        <div id="coordenador">
            O COORDENADOR DO CURSO <br><br>
            _______________________________ <br>
            {{mb_strtoupper(App\Models\Coordenador::pegarCoordenadorDeCurso($dados_turma->curso_id)->nome)}}
        </div>

        <div id="director-turma">
            O DIRECTOR DE TURMA <br><br>
            _______________________________ <br> <br>
        </div>
    </footer>
    <script>
        
    </script>
</body>
</html>
